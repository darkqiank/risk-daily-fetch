...
def parse_directory(url):
    ...
    - 链接通常包含'article'或'post'等关键字
    - 返回格式：links = [...] 的Python列表, 每个link为绝对url
    - 你的返回只包含python代码不要有额外信息
    HTML内容片段：
...
def parse_article(url):
    ...
    if match:
        ...
        # 3. 执行生成的代码
        try:
            local_vars = {'html': html}
            exec(code_block, {'BeautifulSoup': BeautifulSoup}, local_vars)
            return local_vars.get('article', {})
        except Exception as e:
            print(f"正文提取代码执行失败: {str(e)}")
            return {}
    else:
        return {}
...
if __name__ == "__main__":
    ...
    # 正文解析示例
    # if article_links:
    #     first_article = parse_article(article_links[0])
